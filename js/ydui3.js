...
! function(window) {
	...
	CitySelect.prototype.bindItemEvent = function() {
		...
		$cityElement.on('click.ydui.cityselect', '.cityselect-item-box a', function() {
			...
			_this.setNavTxt(tag, $this.text(), $this.find('span').attr('title'));
			...
			if (tag == 0) {
				...
				defaultSet.pid = $nav.eq(0).attr('title');
				defaultSet.cid = $nav.eq(1).attr('title');
				defaultSet.aid = $nav.eq(2).attr('title');
				_this.returnValue();
			}
		});
	};
	...
	CitySelect.prototype.loadProvance = function() {
		...
		$.each(_this.citys, function(k, v) {
			arr.push($('<a class="' + (v.n == _this.defaultSet.provance ? 'crt' : '') + '" href="javascript:;"><span title="' +
				v.n.split(",")[1] + '">' + v.n.split(",")[0] + '</span></a>').data({
				citys: v.c,
				tag: 0
			}));
		});
	...
	CitySelect.prototype.loadCity = function() {
		...
		$.each(cityData, function(k, v) {
			arr.push($('<a class="' + (v.n == _this.defaultSet.city ? 'crt' : '') + '" href="javascript:;"><span title="' + v
				.n.split(",")[1] + '">' + v.n.split(",")[0] + '</span></a>').data({
				citys: v.a,
				tag: 1
			}));
		});
	...
	CitySelect.prototype.loadArea = function() {
		...
		$.each(areaData, function(k, v) {
			arr.push($('<a class="' + (v == _this.defaultSet.area ? 'crt' : '') + '" href="javascript:;"><span title="' + v.split(
				",")[1] + '">' + v.split(",")[0] + '</span></a>').data({
				tag: 2
			}));
		});
	...
	dialog.confirm = function(title, mes, opts) {
		...
		$.each(btnArr, function(i, val) {
			...
			if (typeof val.color == 'boolean') {
				$btn = $('<a href="javascript:;" class="' + 'confirm-btn ' + (val.color ? 'primary' : 'default') + '">' + (val.txt ||
					'') + '</a>');
...
! function(window) {
	...
	InfiniteScroll.prototype.bindScrollEvent = function() {
		...
		$binder.bind('scroll.ydui.infinitescroll', function() {
			...
			if (_this.$tag.offset().top <= contentTop + contentHeight + contentHeight / 3) {
				_this.loadList();
			}
		});
	};
...
! function(window) {
	...
	var $body = $(window.document.body),
		isMobile = !!(window.navigator && window.navigator.userAgent || '').match(/AppleWebKit.*Mobile.*/) || 'ontouchstart' in
	...
	KeyBoard.prototype.createNumsHtml = function() {
		...
		$.each(nums, function(k) {
			if (k % 3 == 0) {
				if (k >= nums.length - 2) {
					arr.push('<li><a href="javascript:;" class="J_Cancel">取消</a>' + nums.slice(k, k + 3).join('') +
						'<a href="javascript:;" class="J_Backspace"></a></li>');
				} else {
...
! function(window) {
	...
	Slider.prototype.createBullet = function() {
		...
		_this.$pagination.append(initActive + new Array(_this.itemNums).join('<span class="' + _this.options.bulletClass +
			'"></span>'));
	};
